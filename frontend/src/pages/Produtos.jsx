...
function Produtos() {
  ...
  return (
    <Container className="p-4">
      ...
      {produtoCat.map((p, id) =>
        <ProductCard key={id} imageSrc={p.images[0]} title={p.title} price={p.price} productSlug={p.slug}/>
      )}
      </div> :
      <div className="text-center flex flex-col h-60 justify-center items-center">
        ...
        <h1 className="font-viminalis">Ainda não temos produtos com esse tema!</h1>
      </div>
      }
    </Container>
  )
