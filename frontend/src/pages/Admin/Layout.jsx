...
function AdminLayout() {
  ...
  // Busca todas as categorias ao carregar a página
  useEffect(() => {
    fetch('http://localhost:3000/api/categorias')
      .then(res => res.json())
      .then(data => {
        // Ordena localmente as que são destacadas por 'featuredOrder'
        const ordenadas = (data.categorias || []).sort(
  ...
  const handleToggleFeatured = async (id, statusAtual) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/categorias/${id}/destaque`,
        {
          method: 'PUT',
          headers: {
  ...
  // Move Categoria para Cima ou para Baixo na lista
  ...
  //Envia a ordem atual da tela para salvar definitivamente no Banco de Dados
  const handleSalvarOrdenacao = async () => {
    ...
    try {
      const response = await fetch(
        'http://localhost:3000/api/categorias/reordenar-destaques',
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        toast.success('Ordenação das categorias salva com sucesso! 🎉');
      }
    } catch (err) {
  ...
  const handleDeleteCategory = async id => {
    if (!(await confirm('Tem certeza que deseja apagar?'))) return;
  ...
  const handleCreateCategory = async e => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Erro: ${errorData.erro}`);
      }
    } catch (err) {
  ...
  const handleToggleActive = async hero => {
    ...
    setHeros(
      heros.map(h => (h.id === hero.id ? { ...h, isActive: updatedStatus } : h))
    );
  };

  // Move Hero para Cima ou para Baixo
  ...
  // Envia a nova ordem das Heroes para o Banco de Dados
  const handleSalvarOrdenacaoHeros = async () => {
    ...
    try {
      const response = await fetch(
        'http://localhost:3000/api/heros/reordenar',
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        toast.success('Ordenação dos banners salva com sucesso! 🎉');
      }
    } catch (err) {
  ...
  // Estados para o Modal de Heroes
  const [isHeroModalOpen, setIsHeroModalOpen] = useState(false);
  const [heroImageFile, setHeroImageFile] = useState(null);

  // Cria a hero no banco
  const handleCreateHero = async e => {
    e.preventDefault();

    if (!heroImageFile) return toast.error('Selecione uma imagem!');
  ...
  return (
    <main className="font-viminalis bg-gray-200 min-h-screen flex flex-col p-4 gap-4">
      ...
      <div className="bg-white rounded-xl flex flex-col p-4 min-w-80 lg:min-w-120 w-fit gap-3">
        <div className="flex justify-between items-center w-full">
          <p className="flex items-center gap-0.5 text-xl pr-2">
            <FaImage className="text-lfapink w-8" />
        ...
        {/* Mapeando os heroes vindos do banco */}
        {heros.map((hero, index) => (
          <div
            key={hero.id}
            className="bg-gray-200 rounded-lg p-2 flex justify-between items-center mt-2 gap-4"
          >
            <img
              className="w-64 rounded-lg aspect-video object-cover"
              src={hero.image}
              alt="Hero"
            />
      ...
      <div className="bg-white rounded-xl flex flex-col p-4 min-w-80 lg:min-w-120 w-fit gap-3">
        <div className="flex justify-between items-center w-full">
          <p className="flex items-center gap-0.5 text-xl pr-2">
            <FaStar className="text-lfapink w-8" />
        ...
        {categorias.map((categoria, index) => (
          <div
            key={categoria.id}
            className="bg-gray-200 rounded-lg p-2 flex justify-between items-center gap-4"
          >
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={() =>
                  handleToggleFeatured(categoria.id, categoria.isFeatured)
                }
              />
              <span
                className={
