...
function AdminVendas() {
  ...
  useEffect(() => {
    ...
    // Assumindo que a sua rota de vendas no backend é /api/sales
  ...
  const handleBuscaCliente = async (nome) => {
    ...
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`http://localhost:3000/api/clientes/search/${nome}?limit=5`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const openModal = (venda = null) => {
    if (venda) {
      ...
      setEditingId(null);
      setFormData({
        canal_venda: 'whatsapp', status: 'aguardando_pagamento', produtos_texto: '', 
        valor_total: '', valor_gastos: '0.00', observacoes: ''
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(errorData.erro || "Erro ao deletar a venda.");
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // 🔴 MUDANÇA IMPORTANTE: Usando FormData em vez de JSON para suportar upload!
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(errorData.erro || "Erro ao salvar a venda.");
      }
    } catch (err) {
  ...
  const getBadgeStyle = (canal) => {
    switch(canal) {
      ...
      case 'artesanou': 
        return { background: 'linear-gradient(to right, #e84d28, #9d3c88)', color: 'white' };
      case 'akeba': 
  ...
  return (
    <main className="font-viminalis bg-gray-200 min-h-screen flex flex-col p-4 gap-4 pb-12">
      ...
      <div className="flex flex-col gap-3">
        {vendas.map((v) => (
          <div key={v.id} className="bg-white rounded-xl flex justify-between items-center p-4 min-w-80 lg:min-w-100 shadow-sm border-l-4 border-lfapink">
            <div className="flex flex-col">
              <p className="font-bold text-xl">{v.Customer?.name || 'Cliente Deletado'}</p>
              <p>{new Date(v.data_venda).toLocaleDateString('pt-BR', {
                  day: '2-digit',
                  month: '2-digit',
                  year: 'numeric',
                  hour: '2-digit',
              ...
              <div className="flex gap-4 items-center mt-1">
                ...
                <span className="text-sm font-semibold text-gray-600 border px-2 py-1 rounded">{v.status.replace('_', ' ')}</span>
                {v.link_xml_nf && (
                  <a href={v.link_xml_nf} target="_blank" rel="noreferrer" className="text-green-600 hover:underline flex items-center gap-1 text-sm font-semibold">
                    <FaFileCode /> Nota Fiscal
                  </a>
                )}
              </div>
              <p className="text-gray-600 mt-2 text-sm max-w-2xl line-clamp-2">{v.produtos_texto}</p>
              <p className="text-lg font-bold text-lfapink mt-2">Total: R$ {Number(v.valor_total).toFixed(2)}</p>
            </div>
            
            <div className="flex gap-3 text-xl">
              <button onClick={() => openModal(v)} className="text-gray-500 hover:brightness-75 transition-all">
                <FaPencilAlt />
              </button>
              <button onClick={() => handleDelete(v.id)} className="text-lfapink hover:brightness-75 transition-all">
                <FaTrash />
              </button>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl w-full max-w-2xl flex flex-col gap-4 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1 relative">
                <label className="text-sm font-semibold text-lfapink">Cliente *</label>
                <input 
                  type="text" 
                  value={buscaNome} 
                  ...
                  <ul className="absolute top-16.25 left-0 w-full bg-white border border-gray-200 rounded shadow-lg z-10 max-h-40 overflow-y-auto">
                    {clientesSugeridos.map(c => (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-500">{c.whatsapp} | {c.cpf}</p>
                      </li>
                    ))}
                  </ul>
                )}
              ...
              <div className="flex gap-4">
                <div className="flex flex-col gap-1 w-1/2">
                  ...
                  <select value={formData.canal_venda} onChange={(e) => setFormData({...formData, canal_venda: e.target.value})} className="border rounded p-2 outline-none focus:border-lfapink bg-white">
                ...
                <div className="flex flex-col gap-1 w-1/2">
                  ...
                  <select value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})} className="border rounded p-2 outline-none focus:border-lfapink bg-white">
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea required rows="3" value={formData.produtos_texto} onChange={(e) => setFormData({...formData, produtos_texto: e.target.value})} className="border rounded p-2 outline-none focus:border-lfapink" placeholder="Ex: 2x Canecas, 1x Topo de Bolo..." />
              </div>

              <div className="flex gap-4">
                <div className="flex flex-col gap-1 w-1/2">
                  <label className="text-sm font-semibold">Valor Total Pago (R$) *</label>
                  <input required type="number" step="0.01" value={formData.valor_total} onChange={(e) => setFormData({...formData, valor_total: e.target.value})} className="border rounded p-2 outline-none focus:border-lfapink font-bold text-lfapink" />
                </div>
                <div className="flex flex-col gap-1 w-1/2">
                  <label className="text-sm font-semibold">Gastos / Taxas (R$)</label>
                  <input type="number" step="0.01" value={formData.valor_gastos} onChange={(e) => setFormData({...formData, valor_gastos: e.target.value})} className="border rounded p-2 outline-none focus:border-lfapink text-red-500" />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1 bg-blue-50 p-3 rounded border border-blue-200">
                ...
                {/* 👇 Mostra o link da nota atual, se estiver editando e ela existir */}
                {editingId && formData.link_xml_nf && (
                  <p className="text-xs text-green-700 mb-2 bg-green-100 p-1 px-2 rounded w-fit">
                    ✓ Nota atual já enviada. <a href={formData.link_xml_nf} target="_blank" rel="noreferrer" className="font-bold hover:underline">Ver arquivo</a>
                  </p>
                )}
                ...
                <p className="text-xs text-blue-600 mt-1">
                  {editingId && formData.link_xml_nf ? "Selecione um arquivo apenas se quiser substituir a nota atual." : "Nenhum arquivo XML anexado ainda."}
                </p>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-sm font-semibold">Observações Internas</label>
                <textarea rows="2" value={formData.observacoes} onChange={(e) => setFormData({...formData, observacoes: e.target.value})} className="border rounded p-2 outline-none focus:border-lfapink" placeholder="Ex: Cliente pediu urgência..." />
              </div>

              {/* --- BOTÕES --- */}
              <div className="flex justify-end gap-2 mt-4 pt-2 border-t">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 transition-all font-semibold cursor-pointer">
                  Cancelar
                </button>
                <button type="submit" className="px-4 py-2 bg-lfapink text-white rounded hover:brightness-90 transition-all font-semibold cursor-pointer">
                  Salvar
                </button>
              </div>
